...
- [ ] 3. **RED-PHASE — orchestrator routes to RED sub-agent**:
  - **Test file (SC-5):** `.opencode/tests/viewport-editor/65/test_session_injected_agents_field.py` — structural assertion: `Session` dataclass has `injected_agents_files: set[str]`
  - **Test file (SC-12):** `.opencode/tests/viewport-editor/65/test_find_sibling_agents_realpath.py` — structural assertion: `_find_sibling_agents_md` calls `os.path.realpath()` on both `file_path` and `project_root`
...
- [ ] 3. **RED-PHASE — orchestrator routes to RED sub-agent**:
  - **Test file (SC-1):** `.opencode/tests/viewport-editor/65/test_viewport_open_injection.py` — behavioral test: open a file under a directory with AGENTS.md → response contains `<system-reminder>` with AGENTS.md content
  - **Test file (SC-2):** `.opencode/tests/viewport-editor/65/test_dedup_same_session.py` — behavioral test: open two files under same AGENTS.md → only first response has injection
  - **Test file (SC-3):** `.opencode/tests/viewport-editor/65/test_root_agents_fallback.py` — behavioral test: open file in dir without AGENTS.md but with one at project root → root AGENTS.md injected
  - **Test file (SC-4):** `.opencode/tests/viewport-editor/65/test_nearest_ancestor_wins.py` — behavioral test: file under `a/b/AGENTS.md` where `a/AGENTS.md` also exists → `a/b/AGENTS.md` injected
  - **Test file (SC-6):** `.opencode/tests/viewport-editor/65/test_absolute_path_skip.py` — behavioral test: open `/etc/passwd` → no `<system-reminder>` in response
  - **Test file (SC-7):** `.opencode/tests/viewport-editor/65/test_unreadable_agents_skip.py` — behavioral test: AGENTS.md with 000 permissions → injection silently skipped
  - **Test file (SC-8):** `.opencode/tests/viewport-editor/65/test_self_injection_guard.py` — behavioral test: open `AGENTS.md` itself → no self-injection
  - **Test file (SC-11):** `.opencode/tests/viewport-editor/65/test_injection_format.py` — string test: grep response for `<system-reminder>\nInstructions from:` pattern
...
**Files:** `.opencode/tests/viewport-editor/65/`
...
- [ ] 3. **RED-PHASE — orchestrator routes to RED sub-agent**:
  - **Test file (SC-9):** `.opencode/tests/viewport-editor/65/test_read_file_injection.py` — behavioral test: `read_file` on a file under AGENTS.md → response contains AGENTS.md content in `<system-reminder>`
  - **Test file (SC-10):** `.opencode/tests/viewport-editor/65/test_cross_tool_dedup.py` — behavioral test: `read_file` then `viewport:open` on files under same AGENTS.md → single injection total across both tools
