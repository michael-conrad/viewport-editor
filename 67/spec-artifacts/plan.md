...
### Task 1.1: RED — Verify stale tag exists

- `grep` for `@v0.2.0` in README.md
- Confirm exactly 2 occurrences
- Record as RED evidence: `grep -c '@v0.2.0' README.md` returns 2

**RED condition:** Both `@v0.2.0` references exist (stale state confirmed).
...
### Task 2.1: RED — Read current tool table section

- Read lines 49-60 (current "6-Tool Surface" table)
- Confirm stale state: missing 5 tools, wrong `switch-mode` action
- Record RED evidence

**RED condition:** The tool table has 6 rows, missing `clipboard`, `read_file`, `write_file`, `edit_text`, `find_text`. The viewport row has `switch-mode` instead of `set-display-mode`. The file row lacks `delete`.
...
### Task 3.1: RED — Confirm section does not exist

- Search README.md for "Consuming Repo Instructions" header
- Confirm absent

**RED condition:** No `Consuming Repo Instructions` section in README.md.
