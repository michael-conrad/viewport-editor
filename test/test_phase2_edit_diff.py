...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc9_edit_replace_stages_in_buffer_autosave_off(client_session: ClientSession, test_project_root: Path) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport",
        arguments={"action": "open", "session_id": "test-sc9", "file_path": file_path_str, "autosave": False},
    )
    ...
    assert "error" not in text.lower(), f"RED FAIL: unexpected error in response: {text[:200]}"
    ...
    list_result = await client_session.call_tool(
        "viewport", arguments={"action": "list", "session_id": "test-sc9"},
    )
    assert "dirty: True" in _get_text(list_result), "RED FAIL: dirty flag not set after edit"
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc10_diff_show_returns_unified_diff(client_session: ClientSession) -> None:
    ...
    """
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc10", "file_path": "edit_test.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    await client_session.call_tool(
        "edit",
        arguments={
            "action": "replace", "session_id": "test-sc10", "viewport_id": vpid,
            "file_path": "edit_test.txt", "old_text": "line 3", "new_text": "CHANGED",
        },
    )

    result = await client_session.call_tool(
        "diff",
        arguments={"action": "show", "session_id": "test-sc10", "viewport_id": vpid, "file_path": "edit_test.txt"},
    )
    text = _get_text(result)
    # GREEN assertion: unified diff format
    assert not result.isError, f"RED FAIL (diff stub not implemented): {text[:200]}"
    assert "---" in text or "+++" in text, f"RED FAIL: unified diff markers missing: {text[:200]}"
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc11_file_save_rejects_stale_mtime(client_session: ClientSession, test_project_root: Path) -> None:
    ...
    """
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc11", "file_path": "save_test.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "file",
        arguments={"action": "save", "session_id": "test-sc11", "viewport_id": vpid, "file_path": "save_test.txt"},
    )
    text = _get_text(result)
    # GREEN assertion: isError=true with staleness warning or force override needed
    assert result.isError, f"RED FAIL: save should reject stale mtime, got: {text[:200]}"
    assert "mtime" in text.lower() or "stale" in text.lower() or "conflict" in text.lower(), (
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc11_file_save_rejects_missing_file(client_session: ClientSession) -> None:
    """SC-11: file:save on missing file returns isError."""
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc11b", "file_path": "save_test.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))

    result = await client_session.call_tool(
        "file",
        arguments={"action": "save", "session_id": "test-sc11b", "viewport_id": vpid, "file_path": "nonexistent.txt"},
    )
    text = _get_text(result)
    assert result.isError, f"RED FAIL: save should reject missing file, got: {text[:200]}"
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc11_file_save_force_overrides_stale(client_session: ClientSession, test_project_root: Path) -> None:
    """SC-11: file:save with force=true overrides stale mtime/size check."""
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc11c", "file_path": "save_test.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "file",
        arguments={
            "action": "save", "session_id": "test-sc11c", "viewport_id": vpid,
            "file_path": "save_test.txt", "force": True,
        },
    )
    text = _get_text(result)
    ...
    diff_result = await client_session.call_tool(
        "diff", arguments={"action": "show", "session_id": "test-sc11c", "viewport_id": vpid, "file_path": "save_test.txt"},
    )
...
async def test_sc12_file_discard_reverts_buffer(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport",
        arguments={"action": "open", "session_id": "test-sc12", "file_path": "edit_test.txt", "autosave": False},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    await client_session.call_tool(
        "edit",
        arguments={
            "action": "replace", "session_id": "test-sc12", "viewport_id": vpid,
            "file_path": "edit_test.txt", "old_text": "line 3", "new_text": "DIRTY",
        },
    )
    ...
    list_result = await client_session.call_tool(
        "viewport", arguments={"action": "list", "session_id": "test-sc12"},
    )
    assert "dirty: True" not in _get_text(list_result), \
        "RED FAIL: discard should clear dirty flag"
    # After discard, diff should be empty
    diff_result = await client_session.call_tool(
        "diff", arguments={"action": "show", "session_id": "test-sc12", "viewport_id": vpid, "file_path": "edit_test.txt"},
    )
    diff_text = _get_text(diff_result)
    assert "no pending changes" in diff_text.lower() or diff_text.strip() == "", \
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc13_edit_replace_autosave_on_writes_to_disk(client_session: ClientSession, test_project_root: Path) -> None:
    """SC-13: edit:replace with autosave=on writes to disk atomically.
    ...
    result_open = await client_session.call_tool(
        "viewport",
        arguments={"action": "open", "session_id": "test-sc13", "file_path": file_path_str, "autosave": True},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "replace", "session_id": "test-sc13", "viewport_id": vpid,
            "file_path": file_path_str, "old_text": "line 3", "new_text": "AUTO SAVED",
        },
    )
    text = _get_text(result)
...
async def test_sc18_replace_all_multi_occurrence(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc18", "file_path": "multi.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "replace-all", "session_id": "test-sc18", "viewport_id": vpid,
            "file_path": "multi.txt", "old_text": "apple", "new_text": "orange",
        },
    )
    text = _get_text(result)
    ...
    diff_result = await client_session.call_tool(
        "diff", arguments={"action": "show", "session_id": "test-sc18", "viewport_id": vpid, "file_path": "multi.txt"},
    )
    diff_text = _get_text(diff_result)
    # The diff should show 3 removals of apple lines and 3 additions of orange lines
    assert "-apple" in diff_text, f"RED FAIL: diff should show apple removal: {diff_text[:200]}"
    assert "+orange" in diff_text, f"RED FAIL: diff should show orange addition: {diff_text[:200]}"
...
async def test_sc19_insert_lines_at_position(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc19", "file_path": "lines.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "insert-lines", "session_id": "test-sc19", "viewport_id": vpid,
            "file_path": "lines.txt", "line_start": 3, "lines": ["inserted A", "inserted B"],
        },
    )
    text = _get_text(result)
    ...
    scroll_result = await client_session.call_tool(
        "viewport", arguments={"action": "scroll", "session_id": "test-sc19", "viewport_id": vpid, "lines": 0},
    )
    scroll_text = _get_text(scroll_result)
    assert "inserted A" in scroll_text, f"RED FAIL: inserted content not visible: {scroll_text[:200]}"
    assert "inserted B" in scroll_text, f"RED FAIL: inserted content not visible: {scroll_text[:200]}"
...
async def test_sc20_delete_lines_at_position(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc20", "file_path": "lines.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "delete-lines", "session_id": "test-sc20", "viewport_id": vpid,
            "file_path": "lines.txt", "line_start": 2, "line_end": 4,
        },
    )
    text = _get_text(result)
    ...
    diff_result = await client_session.call_tool(
        "diff", arguments={"action": "show", "session_id": "test-sc20", "viewport_id": vpid, "file_path": "lines.txt"},
    )
    diff_text = _get_text(diff_result)
    assert "-one" in diff_text, f"RED FAIL: diff should show deleted line 'one': {diff_text[:200]}"
    assert "-two" in diff_text, f"RED FAIL: diff should show deleted line 'two': {diff_text[:200]}"
    assert "-three" in diff_text, f"RED FAIL: diff should show deleted line 'three': {diff_text[:200]}"
...
async def test_sc21_swap_lines(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc21", "file_path": "lines.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "swap-lines", "session_id": "test-sc21", "viewport_id": vpid,
            "file_path": "lines.txt", "line_start": 1, "line_end": 2,
            "target_line_start": 4, "target_line_end": 5,
        },
    )
    text = _get_text(result)
    ...
    scroll_result = await client_session.call_tool(
        "viewport", arguments={"action": "scroll", "session_id": "test-sc21", "viewport_id": vpid, "lines": 0},
    )
    scroll_text = _get_text(scroll_result)
    # Original: zero/one/two/three/four/five... After swap: three/four/two/zero/one/five...
    assert "    1: three" in scroll_text or "    2: three" in scroll_text or "three" in scroll_text, \
        f"RED FAIL: swapped content not visible: {scroll_text[:300]}"
    assert "    1: zero" not in scroll_text, f"RED FAIL: zero should no longer be at line 1: {scroll_text[:300]}"
...
async def test_sc22_move_lines(client_session: ClientSession) -> None:
    ...
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc22", "file_path": "lines.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))
    ...
    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "move-lines", "session_id": "test-sc22", "viewport_id": vpid,
            "file_path": "lines.txt", "line_start": 5, "line_end": 7, "target_line": 2,
        },
    )
    text = _get_text(result)
    ...
    scroll_result = await client_session.call_tool(
        "viewport", arguments={"action": "scroll", "session_id": "test-sc22", "viewport_id": vpid, "lines": 0},
    )
    scroll_text = _get_text(scroll_result)
    ...
    assert four_pos >= 0, f"RED FAIL: moved line 'four' not visible: {scroll_text[:300]}"
    assert one_pos >= 0, f"RED FAIL: expected line 'one' still visible: {scroll_text[:300]}"
    assert four_pos < one_pos, f"RED FAIL: 'four' should appear before 'one' after move: {scroll_text[:300]}"
...
@pytest.mark.phase2
@pytest.mark.asyncio
async def test_sc25_soft_conflict_warning_on_edit(client_session: ClientSession, test_project_root: Path) -> None:
    ...
    """
    result_open = await client_session.call_tool(
        "viewport", arguments={"action": "open", "session_id": "test-sc25", "file_path": "conflict_test.txt"},
    )
    vpid = _extract_vpid(_get_text(result_open))

    (test_project_root / "conflict_test.txt").write_text("externally modified\nstill modified\n")
    time.sleep(0.05)

    result = await client_session.call_tool(
        "edit",
        arguments={
            "action": "replace", "session_id": "test-sc25", "viewport_id": vpid,
            "file_path": "conflict_test.txt", "old_text": "original content", "new_text": "conflicting edit",
        },
    )
    text = _get_text(result)
