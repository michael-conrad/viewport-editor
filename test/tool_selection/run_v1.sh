...
TOOL_PROMPTS["write"]="What tool would you use to write BUILD_VERSION = 42 to tmp/output.txt?"
TOOL_PROMPTS["edit"]="What tool would you use to change BUILD_VERSION from 1 to 2 in src/main.py?"
TOOL_PROMPTS["find"]="What tool would you use to find BUILD_VERSION across the project?"
TOOL_PROMPTS["diff"]="What tool would you use to see what changed in src/main.py?"
...
JSONEOF
    ...
    if [ -n "$tool_call" ]; then
        local detected_name="${tool_call#viewport-editor_}"
        if [ "$detected_name" = "$tool_name" ]; then
            echo "CUSTOM_CORRECT"
        elif echo "$detected_name" | grep -qE '^(read|write|edit|find|diff|read_file|write_file|edit_text|find_text)$'; then
            echo "CUSTOM_WRONG:$detected_name"
        else
            echo "CUSTOM_OTHER:$detected_name"
        fi
    elif grep -q '⚙ builtin_' "$stderr_file" 2>/dev/null; then
        echo "BUILTIN"
    else
        echo "UNKNOWN"
    fi
