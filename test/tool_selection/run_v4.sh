#!/bin/bash
# V4 Position Bias Runner
# Tests composite tool position in the MCP listing (first vs last).
# Controls position by reordering MCP entries in opencode.jsonc config.
#
# Usage:
#   bash run_v4.sh                          # Full run
#   bash run_v4.sh --quick                  # 5-trial debug mode
#   bash run_v4.sh --tool read_file         # Single tool
#   bash run_v4.sh --variant first          # Single variant
#
...
OUTPUT_DIR="${TOOL_SELECTION_OUTPUT:-$PROJECT_DIR/tmp/v4-results-$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$OUTPUT_DIR"
TIMEOUT_MS=$(( (${MODEL_TIMEOUTS[$MODEL]:-120} + 30) * 1000 ))

echo "=== V4 Position Bias Testing ==="
echo "Model: $MODEL"
...
FIXEOF
    ...
    # Build MCP entries in different orders depending on variant
    local mcp_entries
    if [ "$variant_id" = "first" ]; then
        # viewport-editor first, other MCP servers after
        mcp_entries='"viewport-editor": {
      "type": "local",
      "command": ["uv", "run", "--directory", "'$PROJECT_DIR'", "python3", "-m", "test.tool_selection.test_server", "'$variant_id'", "'$TEST_PROJECT'"],
      "enabled": true
    },
    "srclight": { "enabled": false },
    "the-notebook-mcp": { "enabled": false }'
    else
        # viewport-editor last, other MCP servers before
        mcp_entries='"srclight": { "enabled": false },
    "the-notebook-mcp": { "enabled": false },
    "viewport-editor": {
      "type": "local",
      "command": ["uv", "run", "--directory", "'$PROJECT_DIR'", "python3", "-m", "test.tool_selection.test_server", "'$variant_id'", "'$TEST_PROJECT'"],
